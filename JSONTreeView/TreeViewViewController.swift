...
class TreeViewViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        let pathBBI = UIBarButtonItem(title: "Path", style: .done, target: self, action: #selector(TreeViewViewController.getFullPath))
    ...
    func getFullPath() {
        ...
        let alertC = UIAlertController(title: "Path", message: path, preferredStyle: .alert)
        let copyAction = UIAlertAction(title: "Copy", style: .destructive, handler: { (_) in
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        guard let dict = dataSource as? Dictionary<String, Any> else {
            guard let arr = dataSource as? Array<AnyHashable> else {
                let alertC = UIAlertController(title: "Detail", message: tableView.cellForRow(at: indexPath)?.textLabel?.text, preferredStyle: .alert)
                let copyAction = UIAlertAction(title: "Copy", style: .destructive, handler: { (_) in
                    let pasteboard = UIPasteboard.general
                    pasteboard.string = alertC.message!
                })
